...
| Reviewed PDI-H-COOH C 1s publication figure | `python examples/plot_pdi_h_cooh_c1s_publication.py --sample-manifest PATH` | `figures/final/PDI-H-COOH/pdi_h_cooh_c1s_publication.{png,pdf}` |

Fitting examples use deterministic synthetic spectra. The dedicated experimental
publication example is plotting-only and requires a calibrated reviewed sample manifest; no raw
