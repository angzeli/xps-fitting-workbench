...
def test_end_to_end_plot_sources_are_numerically_consistent(tmp_path) -> None:
    ...
    config = FitConfig("integration", "C 1s", [PeakConfig("aromatic_C-C_C=C", 284.8, (284.4, 285.4), 110, fwhm=1.4, fwhm_bounds=(1.0, 1.8), line_shape="gaussian")])
    ...
    serialised_path = tmp_path / "full_result.json"; serialised_path.write_text(json.dumps(fitted.to_dict()))
    ...
    for candidate in (from_csv, from_xlsx, from_json, from_memory_dict):
        ...
        figure, axis = plot_xps_fit(candidate, component_style="hidden")
