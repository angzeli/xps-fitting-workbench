...
def test_recipe_round_trip_and_validation(tmp_path) -> None:
    config = PlotConfig(theme="presentation", output_formats=("svg",), output_filename="recipe", residual_panel=True, x_limits=(2, 0))
...
def test_curve_and_serialised_loading_and_cli(tmp_path) -> None:
    ...
    recipe = PlotConfig(output_formats=("svg",), output_filename="cli").save(tmp_path / "recipe.json")
    assert main(["plot", str(csv), "--metadata", str(metadata), "--recipe", str(recipe), "--output-dir", str(tmp_path)]) == 0
    assert (tmp_path / "cli.svg").stat().st_size > 100
