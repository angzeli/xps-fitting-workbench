...
def main(argv: list[str] | None = None) -> int:
    ...
    table = pd.DataFrame({"binding_energy_eV": dataframe_spectrum.binding_energy, "intensity": dataframe_spectrum.intensity})
    csv_path, xlsx_path = output / "synthetic_c1s.csv", output / "synthetic_c1s.xlsx"
    table.to_csv(csv_path, index=False); table.to_excel(xlsx_path, index=False)
    ...
    print(f"DataFrame/CSV/XLSX: {csv_spectrum.binding_energy.size}/{xlsx_spectrum.binding_energy.size} ordered synthetic points; {vgd_summary}")
