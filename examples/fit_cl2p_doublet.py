...
def main(argv: list[str] | None = None) -> int:
    ...
    output = prepare_output(args.output_dir); energy = np.linspace(195, 207, 481)
    intensity = 10 + pseudo_voigt(energy, 1000, 200, 1.2, 0.5) + pseudo_voigt(energy, 500, 201.6, 1.2, 0.5)
    result = fit_spectrum(Spectrum(energy, intensity, region="Cl 2p", metadata={"data_origin": "deterministic synthetic"}), FitConfig("Cl_doublet", "Cl 2p", cl2p_doublet("Cl", 199.8, 900)))
    figure, _ = plot_xps_fit(result, core_level="Cl 2p", sample_label="Synthetic Cl 2p", show_residual=True)
    paths = export_figure(figure, output / "cl2p_constrained_doublet", formats=("png", "pdf"))
    print(result.fit_statistics); print("Created:", ", ".join(map(str, paths.values())))
    return 0
